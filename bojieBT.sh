...
MAIN_RETURN=${Red_font}[宝塔面板 v6.1.2 破解脚本]${Font_suffix}
git_check(){
    GIT_V=`git --version | grep -q version`
if [ $? = 0 ]; thenecho -e "${Info} Git 已安装，开始下一步操作!"
elseecho -e "${Error} 未安装 Git，开始安装 Git!"
    check_system
    install_git
fi
}
check_system() {
    if grep -Eqii "CentOS" /etc/issue || grep -Eq "CentOS" /etc/*-release; then
        DISTRO='CentOS'
        PM='yum'elif grep -Eqi "Red Hat Enterprise Linux Server" /etc/issue || grep -Eq "Red Hat Enterprise Linux Server" /etc/*-release; then
        DISTRO='RHEL'
        PM='yum'elif grep -Eqi "Aliyun" /etc/issue || grep -Eq "Aliyun" /etc/*-release; then
        DISTRO='Aliyun'
        PM='yum'elif grep -Eqi "Debian" /etc/issue || grep -Eq "Debian" /etc/*-release; then
        DISTRO='Debian'
        PM='apt'elif grep -Eqi "Ubuntu" /etc/issue || grep -Eq "Ubuntu" /etc/*-release; then
        DISTRO='Ubuntu'
        PM='sudo'else
        DISTRO='unknow'fi
}
...
copy_class_file(){
    ...
    echo -e "${Info} 复制 Class 文件完成，正在进行下一步操作!"
}
restart_btpanel(){
...
install_btwaf_httpd(){
    ...
    PLUGIN_RETURN=${Red_font}[Apache 防火墙]${Font_suffix}破解完成，继续破解或返回主菜单: && plugin_choose
}
install_total_main(){
...
install_btwaf(){
    ...
    PLUGIN_RETURN=${Red_font}[Nginx 防火墙]${Font_suffix}破解完成，继续破解或返回主菜单: && plugin_choose
}
...
install_masterslave(){
    ...
    PLUGIN_RETURN=${Red_font}[MYSQL 主从复制]${Font_suffix}破解完成，继续破解或返回主菜单: && plugin_choose
}
...
while [[ ! "${choose_function}" =~ ^[1-9]$ ]]
    doecho -e "${Error} 无效输入"echo -e "${Info} 请重新选择" && read -p "输入数字以选择:" choose_function
    done
...
while [[ ! "${function}" =~ ^[1-4]$ ]]
    doecho -e "${Error} 无效输入"echo -e "${Info} 请重新选择" && read -p "输入数字以选择:" functiondone
if [[ "${function}" == "1" ]]; then
    setup
